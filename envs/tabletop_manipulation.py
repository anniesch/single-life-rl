...
class TabletopManipulation(mujoco_env.MujocoEnv):
...
#     print("Resetting Tabletop env!")
    ...
    if self._reset_at_goal:
      ...
      if self.ns_init:
          full_qpos[2:4] = [2.7, 1.5] + np.random.uniform(-0.15, 0.15, size=(2,)) if np.random.random() < 0.5 else [2.7, -1.5] + np.random.uniform(-0.15, 0.15, size=(2,))
          self.goal[2:4] = [-2.5, -1.0] if np.random.random() < 0.5 else [ -2.5,  1.0]
  ...
  def step(self, action):
    ...
    if self.ns_init:
        ...
        done = self.is_successful(next_obs)
  ...
  def is_successful(self, obs=None):
    if obs is None:
      obs = self._get_obs()
    return np.linalg.norm(obs[2:4] - obs[-4:-2]) <= 0.15
